          ...
          <div class="d-flex align-center justify-space-between flex-wrap">
            <div>
              ...
              <p class="text-body-1 mb-0" style="color: #6b7280;">
                Manage customer contacts and send email campaigns on behalf of your clients
              </p>
            </div>
          </div>
        </v-col>
            ...
            <v-card-text class="pa-6">
              ...
              >
                <div class="d-flex align-center">
                  ...
                  <span>No customer contacts found for this client. Add contacts to start sending emails.</span>
                </div>
              </v-alert>
        ...
        <v-col cols="12">
          ...
          >
            <v-card-title 
              ...
              style="background: linear-gradient(135deg, #f9fafb 0%, #ffffff 100%); border-bottom: 1px solid #e5e7eb;"
            >
              <v-icon class="mr-3" color="primary" size="24">mdi-email-send</v-icon>
              ...
              <v-form @submit.prevent="sendEmail" class="form-dialog-fields">
                ...
                >
                  <div class="d-flex align-center">
                    ...
                    <span>Found <strong>{{ availableContacts.length }}</strong> customer contact(s) with email addresses for this client.</span>
                  </div>
                </v-alert>
                ...
                >
                  <div class="d-flex align-center">
                    ...
                    <span>No customer contacts with email addresses found for this client. Add contacts first.</span>
                  </div>
                </v-alert>
                ...
                >
                  <template v-slot:item="{ props, item }">
                    <v-list-item v-bind="props">
                      ...
                      <v-list-item-subtitle style="color: #6b7280;">{{ item.raw.email }}</v-list-item-subtitle>
                    </v-list-item>
                  </template>
                </v-select>
                ...
                <v-btn
                  ...
                  :loading="sending"
                  :disabled="!subject.trim() || !selectedClient || availableContacts.length === 0"
                  class="mt-6"
                  variant="elevated"
...
export default {
  ...
  async mounted() {
    ...
    if (this.$route.query.template) {
      await this.loadTemplate(this.$route.query.template);
    }
  },
  methods: {
    async loadClients() {
    ...
    async loadTemplates() {
      try {
        const response = await axios.get('/templates', { params: { type: 'email' } });
    ...
    async loadTemplate(templateId) {
      ...
      try {
        ...
        this.textContent = template.content || '';
    ...
    async deleteContact(contactId) {
      ...
      this.confirmMessage = `Are you sure you want to delete ${contact?.name || contact?.email || 'this contact'}? This action cannot be undone.`;
    ...
    async sendEmail() {
      if (!this.selectedClient || !this.subject.trim() || this.availableContacts.length === 0) return;
      ...
      try {
        ...
        this.$store.dispatch('showSnackbar', {
          text: `Email sent to ${sentCount} out of ${totalCount} customer contact(s)!`,
          color: 'success'
        });
        ...
        this.$store.dispatch('showSnackbar', {
          text: 'Error sending email: ' + (error.response?.data?.message || error.message),
          color: 'error'
        });
      } finally {
        this.sending = false;
